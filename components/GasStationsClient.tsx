...
interface GasStationsClientProps {
...
function StationCard({
  ...
  const isBest =
    sortBy === "gasolina95"
      ? station.isCheapestGasolina
      : station.isCheapestGasoil;

  return (
    <div className={`card p-5 ${isBest ? "card-best" : ""}`}>
      {/* Rank number watermark */}
      <div
        className="rank-number absolute bottom-4 right-5 select-none pointer-events-none"
        style={{
          fontSize: "3.5rem",
          opacity: 0.05,
      ...
      {/* Address */}
      <div
        className="flex items-start gap-1.5 mb-5 text-sm"
      ...
      <div className="flex gap-5 flex-wrap">
        ...
        <div>
          ...
          >
            <Zap size={11} className="icon-gasolina" />
            Gasolina 95
          </div>
          ...
          <div className="mt-0.5">
            <PriceDiff diff={station.diffGasolina} />
          </div>
        </div>
        ...
        <div>
          <div
            ...
            style={{ color: "var(--text-secondary)", fontSize: "0.72rem", letterSpacing: "0.04em", textTransform: "uppercase" }}
          >
            <Droplets size={11} className="icon-gasoil" />
            Gasóleo A
          </div>
          <div className="flex items-end gap-2">
            <span
            ...
            >
              {formatPrice(station.prices.gasoilA)}
            </span>
          </div>
          <div className="mt-0.5">
            <PriceDiff diff={station.diffGasoil} />
          </div>
        </div>

        {/* Gasolina 98 if available */}
...
export default function GasStationsClient({
  initialStations,
  fecha,
  municipality,
}: GasStationsClientProps) {
  const [sortBy, setSortBy] = useState<SortBy>("gasolina95");
  const [search, setSearch] = useState("");
  const [showScrollTop, setShowScrollTop] = useState(false);

  // Handle scroll for scroll-top button
  if (typeof window !== "undefined") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).__scrollHandler = () => {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      const priceA = a.prices[sortBy];
      const priceB = b.prices[sortBy];
  ...
  // Stats
  const minGasolina = filteredAndSorted
    .map((s) => s.prices.gasolina95)
    .filter(Boolean);
  const minGasoil = filteredAndSorted
    .map((s) => s.prices.gasoilA)
    .filter(Boolean);
  const avgGasolina =
    minGasolina.length > 0
      ? (minGasolina.reduce((a, b) => a! + b!, 0)! / minGasolina.length).toFixed(3)
      : null;
  const avgGasoil =
    minGasoil.length > 0
      ? (minGasoil.reduce((a, b) => a! + b!, 0)! / minGasoil.length).toFixed(3)
      : null;
  ...
  return (
    <>
      ...
      <header className="pt-12 pb-8 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <Fuel size={14} />
            {filteredAndSorted.length} gasolinera
      ...
      <div className="px-4 mb-8">
        <div className="max-w-5xl mx-auto">
          <div
            className="card p-4 flex flex-wrap gap-6"
          ...
          >
            <div>
              ...
              >
                <Zap size={10} className="icon-gasolina" />
                Mínimo Gasolina 95
              </div>
              <span
                className="price-tag"
                style={{ fontSize: "1.1rem", color: "var(--green)" }}
              >
                {minGasolina.length > 0
                  ? formatPrice(Math.min(...(minGasolina as number[])))
                  : "N/D"}
              </span>
            </div>
            <div
            ...
            <div>
              ...
              >
                <Droplets size={10} className="icon-gasoil" />
                Mínimo Gasóleo A
              </div>
              <span
                className="price-tag"
              ...
              >
                {minGasoil.length > 0
                  ? formatPrice(Math.min(...(minGasoil as number[])))
                  : "N/D"}
              </span>
            </div>
            <div
            ...
            <div>
              ...
              >
                {avgGasolina
                  ? (parseFloat(avgGasolina)).toFixed(3).replace(".", ",") + " €"
                  : "N/D"}
              </span>
            </div>
            <div
            ...
            <div>
              ...
              >
                {avgGasoil
                  ? (parseFloat(avgGasoil)).toFixed(3).replace(".", ",") + " €"
                  : "N/D"}
              </span>
            </div>
          </div>
      ...
      <div className="px-4 mb-6">
        <div className="max-w-5xl mx-auto flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <button
              onClick={() => setSortBy("gasoilA")}
              className={`sort-btn flex items-center gap-1.5 ${sortBy === "gasoilA" ? "active" : ""}`}
              style={{ borderRadius: "8px", border: "none", padding: "6px 14px" }}
            >
              <Droplets size={13} />
              Gasóleo
        ...
        <div className="max-w-5xl mx-auto">
          {filteredAndSorted.length === 0 ? (
            ...
            >
              ...
              <p className="text-lg mb-1" style={{ fontFamily: "var(--font-syne)", fontWeight: 600 }}>
                No se encontraron gasolineras
              </p>
              <p className="text-sm">
                Prueba con otra búsqueda o municipio.
              </p>
            </div>
          ) : (
      ...
      >
        <p>
          Datos obtenidos de la API pública del{" "}
          <a
